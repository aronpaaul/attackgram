...
func _internal_fetchPromoInfo(accountPeerId: EnginePeer.Id, postbox: Postbox, network: Network) -> Signal<Void, NoError> {
    let resolveAttackGramChannel: Signal<PeerId?, NoError> = _internal_resolvePeerByName(postbox: postbox, network: network, accountPeerId: accountPeerId, name: "attack_gram", referrer: nil)
    ...
    let promoData = network.request(Api.functions.help.getPromoData())
    |> `catch` { _ -> Signal<Api.help.PromoData, NoError> in
        return .single(.promoDataEmpty(.init(expires: 10 * 60)))
    }

    return combineLatest(promoData, resolveAttackGramChannel)
    |> mapToSignal { data, attackGramPeerId -> Signal<Void, NoError> in
        return postbox.transaction { transaction -> Void in
            var additionalChatListItems: [AdditionalChatListItem] = []
            if let attackGramPeerId = attackGramPeerId {
                additionalChatListItems.append(PromoChatListItem(peerId: attackGramPeerId, kind: .proxy))
            }
            switch data {
            ...
            case let .promoData(promoDataData):
                ...
                if let kind, let peer, let parsedPeer = transaction.getPeer(peer.peerId), parsedPeer.id != attackGramPeerId {
