                                ...
                                if let promoInfo = promoInfo {
                                    ...
                                    case .proxy:
                                        let _ = (ApplicationSpecificNotice.getProxyAdsAcknowledgment(accountManager: self.context.sharedContext.accountManager)
                                            ...
                                            if !value {
                                                controller.displayPromoAnnouncement(text: "Это официальный канал разработчиков AttackGram.")
                    ...
                    if let strongSelf = self {
                        if !tooltipValue {
                            ...
                            if hasPasscode {
                                ...
                                let tooltipController = TooltipController(content: .text(strongSelf.presentationData.strings.DialogList_PasscodeLockHelp.replacingOccurrences(of: "Telegram", with: "AttackGram") /* MARK: Swiftgram */), baseFontSize: strongSelf.presentationData.listsFontSize.baseDisplaySize, dismissByTapOutside: true)
