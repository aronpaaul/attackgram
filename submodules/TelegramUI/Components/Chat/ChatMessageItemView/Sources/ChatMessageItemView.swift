...
open class ChatMessageItemView: ListViewItemNode, ChatMessageItemNodeProtocol {
    ...
    open func setupItem(_ item: ChatMessageItem, synchronousLoad: Bool) {
        ...
        if let deletedAttribute = item.message.attributes.first(where: { $0 is SGDeletedMessageAttribute }) as? SGDeletedMessageAttribute {
            let now = Int32(Date().timeIntervalSince1970)
            let isFresh = now - deletedAttribute.deletionTimestamp <= 3
            if isFresh && self.alpha > 0.9 {
